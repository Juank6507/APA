...
class TogetherProvider(ModelProvider):
    ...
    TOGETHER_MODELS = [
        ...
        },
        {
            ...
            "name": "Qwen2.5 Coder 32B (Together)",
            "context_length": 131072,
            "capabilities": ["long_context","instruction","coding"],
            "quality_score": 90,
            "is_free_tier": True,
        ...
        },
        {
            ...
            "name": "Llama 3.2 11B Vision (Together)",
            "context_length": 131072,
            "capabilities": ["long_context","instruction","vision"],
            "quality_score": 84,
            "is_free_tier": True,
            "provider": "together"
        }
    ...
    def call(self, model_id: str, messages: list[dict], max_tokens: int = 2000, temperature: float = 0.1) -> dict:
        try:
            ...
            if response.status_code == 429:
                logger.warning(f"Together.ai rate limit on model {model_id}")
                return {
                    "content": "",
                    "model_used": model_id,
                    "provider": self.nombre,
            ...
            logger.error(f"TogetherProvider.call error: {str(e)}")
            return {
                "content": "",
                "model_used": model_id,
                "provider": self.nombre,
    ...
    def get_models(self) -> list[dict]:
        return self.TOGETHER_MODELS.copy()
...
class FireworksProvider(ModelProvider):
    ...
    FIREWORKS_MODELS = [
        ...
        },
        {
            ...
            "name": "Qwen2.5 Coder 32B (Fireworks)",
            "context_length": 131072,
            "capabilities": ["long_context","instruction","coding"],
            "quality_score": 90,
            "is_free_tier": True,
    ...
    def call(self, model_id: str, messages: list[dict], max_tokens: int = 2000, temperature: float = 0.1) -> dict:
        try:
            ...
            if response.status_code == 429:
                logger.warning(f"Fireworks.ai rate limit on model {model_id}")
                return {
                    "content": "",
                    "model_used": model_id,
                    "provider": self.nombre,
            ...
            logger.error(f"FireworksProvider.call error: {str(e)}")
            return {
                "content": "",
                "model_used": model_id,
                "provider": self.nombre,
    ...
    def get_models(self) -> list[dict]:
        return self.FIREWORKS_MODELS.copy()


class ArenaRankingsProvider(ModelProvider):
    """
    Arena.ai (LMArena) no es una API de inferencia, sino una plataforma
    de evaluación. Este proveedor NO hace llamadas de inferencia.
    Proporciona rankings de modelos basados en Elo para influir en
    la selección, pero delega la inferencia a otros proveedores.
    """
    nombre = "arena"
    prioridad = 99  # Último, solo para rankings
    
    # Rankings estáticos basados en Chatbot Arena (actualizar periódicamente)
    ARENA_RANKINGS = {
        "claude-3-5-sonnet-20241022": {"elo": 1271, "votes": 45000, "category": "text"},
        "gemini-2.0-flash-exp": {"elo": 1268, "votes": 38000, "category": "text"},
        "gpt-4o-2024-11-20": {"elo": 1265, "votes": 52000, "category": "text"},
        "claude-3-opus-20240229": {"elo": 1255, "votes": 41000, "category": "text"},
        "llama-3.1-405b-instruct": {"elo": 1248, "votes": 28000, "category": "text"},
        "deepseek-v3": {"elo": 1245, "votes": 22000, "category": "coding"},
        "qwen2.5-coder-32b": {"elo": 1238, "votes": 18000, "category": "coding"},
        "mixtral-8x22b-instruct": {"elo": 1230, "votes": 25000, "category": "text"},
    }
    
    def is_available(self) -> bool:
        # Siempre disponible para rankings, no para inferencia
        return True
    
    def call(self, model_id: str, messages: list[dict], max_tokens: int = 2000, temperature: float = 0.1) -> dict:
        # Arena no ofrece inferencia directa - delegar a otro proveedor
        logger.warning(f"ArenaProvider.call() llamado pero Arena no ofrece inferencia. Usar otro proveedor.")
        return {
            "content": "",
            "model_used": model_id,
            "provider": self.nombre,
            "success": False,
            "error": "arena_no_inference"
        }
    
    def get_models(self) -> list[dict]:
        # Retorna modelos con sus rankings de Arena para influencia en selección
        result = []
        for model_id, data in self.ARENA_RANKINGS.items():
            result.append({
                "id": model_id,
                "name": f"{model_id} (Arena Elo: {data['elo']})",
                "context_length": 131072,  # Estimado
                "capabilities": ["coding"] if data["category"] == "coding" else ["instruction"],
                "quality_score": min(100, data["elo"] - 1100),  # Normalizado 0-100
                "is_free_tier": False,
                "provider": self.nombre,
                "arena_elo": data["elo"],
                "arena_votes": data["votes"]
            })
        return result
    
    def get_arena_elo(self, model_id: str) -> Optional[int]:
        """Obtiene el Elo de Arena para un modelo específico."""
        for mid, data in self.ARENA_RANKINGS.items():
...
class ProviderManager:
    def __init__(self):
        self.providers = {
            ...
            "arena": ArenaRankingsProvider()  # Solo para rankings
        }
        self.priority_order = [
            p.strip() for p in
    ...
    def health_check(self) -> dict:
        try:
            ...
            for name, provider in self.providers.items():
                # Saltar Arena para health check de disponibilidad real
                if name == "arena":
                    continue
    ...
    def get_arena_elo_bonus(self, model_id: str) -> float:
        """Obtiene bonus de Elo de Arena para un modelo (0-20 puntos)."""
        arena = self.providers.get("arena")
        if arena and isinstance(arena, ArenaRankingsProvider):
            elo = arena.get_arena_elo(model_id)
            if elo:
                return min(20, (elo - 1100) / 10)  # Normalizado 0-20
        return 0
    ...
    def call_with_fallback(
        ...
        for name, provider in self.providers.items():
            if name == "arena":
                continue  # Arena no hace inferencia
        ...
        for provider_name in self.priority_order:
            if provider_name in ("arena", target_provider):
                continue
