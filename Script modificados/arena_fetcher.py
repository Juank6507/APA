...
# FIX: Corregido typo: era "_arena_ Dict" ahora es "_arena_data: Dict"
...
def _fetch_from_github() -> Optional[Dict[str, Any]]:
    """Intenta cargar desde GitHub como fallback (JSON: {model: score})."""
    try:
        logger.info(f"Intentando cargar desde GitHub: {ARENA_GITHUB_URL}")
        response = requests.get(ARENA_GITHUB_URL, timeout=10)
        response.raise_for_status()
        data = response.json()
        if not isinstance(data, dict):
            logger.warning("GitHub: formato JSON inválido")
            return None
        models_dict = {}
        for model, score in data.items():
            normalized = normalize_model_id(model)
        ...
        logger.info(f"GitHub cargado: {len(models_dict)} modelos")
        return models_dict
        ...
        logger.warning(f"GitHub error: {type(e).__name__}: {e}")
        return None
...
def _parse_arena_csv(csv_text: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # FIX: Detectar columna de modelo por subcadena (insensible a mayúsculas)
        col_model = next((c for c in reader.fieldnames if 'model' in c.lower()), None)
        # FIX: Detectar columna de puntuación por subcadena (insensible a mayúsculas)
        col_elo = next((c for c in reader.fieldnames if any(kw in c.lower() for kw in ['arena', 'score', 'elo'])), None)
        
        if not col_model or not col_elo:
            logger.warning(f"arena_fetcher: Columnas requeridas no encontradas. Model={col_model}, Score={col_elo}. Disponibles: {reader.fieldnames}")
            return None
        ...
        for row in reader:
            ...
            try:
                ...
                elo = 1000.0
            # Usar valores por defecto para category y votes
            models_dict[normalized] = {"elo": elo, "category": "general", "votes": 0}
            parsed_count += 1
        if not models_dict:
...
def _fetch_rankings_with_fallback() -> Optional[Dict[str, Any]]:
    ...
    if raw:
        ...
        return raw
    logger.warning("✗ HuggingFace falló, intentando GitHub...")
    # 2. GitHub (fallback)
    raw = _fetch_from_github()
    if raw:
        _save_cache(raw)
        logger.info("✓ Rankings obtenidos de GitHub (fallback)")
        return raw
    logger.warning("✗ GitHub falló, intentando caché local...")
...
if __name__ == "__main__":
    ...
    # Pruebas de autenticación HF (mocks) - mantener compatibilidad
    print("\n[4] Pruebas de autenticación HuggingFace (mocks)...")

    VALID_CSV = "model,elo,category,votes\nopenai/gpt-4o,1287,general,45230\nanthropic/claude-3-5-sonnet,1271,general,41500\n"

    with patch('core.arena_fetcher.settings') as mock_settings:
        mock_settings.HF_TOKEN = "fake_token_123"

        def mock_get_with_token(url, **kwargs):
            mock_resp = MagicMock()
            mock_resp.status_code = 200
            mock_resp.text = VALID_CSV
            mock_resp.raise_for_status = lambda: None
            headers = kwargs.get('headers', {})
            assert 'Authorization' in headers, "Falta cabecera Authorization con token"
            assert headers['Authorization'] == "Bearer fake_token_123"
            return mock_resp

        with patch('requests.get', side_effect=mock_get_with_token):
            try:
                result = _fetch_from_github()
                print("✓ Token HF enviado correctamente en la petición")
            except Exception as e:
                print(f"⚠️ Advertencia en prueba con token: {e}")

    with patch('core.arena_fetcher.settings') as mock_settings:
        mock_settings.HF_TOKEN = None

        def mock_get_no_token(url, **kwargs):
            mock_resp = MagicMock()
            mock_resp.status_code = 401
            mock_resp.raise_for_status = lambda: None
            headers = kwargs.get('headers', {})
            assert 'Authorization' not in headers, "No debe enviar Authorization sin token"
            return mock_resp

        with patch('requests.get', side_effect=mock_get_no_token):
            try:
                result = _fetch_from_github()
                print("✓ Sin token, la petición se envía sin Authorization")
            except Exception as e:
                print(f"⚠️ Advertencia en prueba sin token: {e}")
