...
# =============================================================================
# SISTEMA DE COSTES DINÁMICOS: Precios obtenidos de provider_manager
# =============================================================================
_price_cache: Dict[str, float] = {}


def _get_model_price_per_token(model: str) -> float:
    """
    Obtiene el precio por token para un modelo desde provider_manager.
    - Consulta provider_manager.get_all_models() (ya cacheada internamente).
    - Busca el modelo por id.
    - Extrae pricing (dict con prompt y completion como strings).
    - Convierte a float, suma prompt + completion.
    - Si no se encuentra pricing, retorna 0.0.
    - Cachea el resultado en memoria para no repetir la búsqueda.
    """
        ...
        for m in all_models:
            if m.get("id") == model:
                ...
                if pricing:
                    ...
                    price_per_token = prompt_price + completion_price
                    _price_cache[model] = price_per_token
                    return price_per_token
                break
    except Exception as e:
        logger.warning(f"Error obteniendo precio para modelo {model}: {e}")

    _price_cache[model] = 0.0
    return 0.0


def _get_equivalent_price(model: str) -> float:
    """
    Retorna el precio por token del modelo con puntuación Arena más cercana
    al modelo dado, que tenga un pricing válido (> 0).
    Si no se encuentra ningún modelo con precio, retorna 0.0.
    """
    from core.arena_fetcher import get_score_for_model
    from core.providers import provider_manager

    target_score = get_score_for_model(model, task_type="generation") or 0.0
    ...
    for m in all_models:
        ...
        score = get_score_for_model(m['id'], task_type="generation") or 0.0
        diff = abs(target_score - score)
        if diff < best_diff:
            best_diff = diff
            best_price = price
...
def _get_dashboard_data(project_id: str) -> dict:
    """Obtiene métricas para el dashboard usando UsageTracker y precios dinámicos."""
    from core.usage_tracker import UsageTracker
    import sqlite3
    from pathlib import Path
    import json
    ...
    # Cálculo de coste real usando price_estimator (precio real o estimado por ranking)
    real_cost_usd = 0.0
    for model, tokens in aggregated_usage.items():
        price = estimate_price(model)
        real_cost_usd += tokens * price
    real_cost_usd *= INFRASTRUCTURE_OVERHEAD_FACTOR

    # Cálculo de coste estimado (misma lógica; la diferencia semántica la maneja el estimador si se desea)
    estimated_cost_usd = 0.0
    for model, tokens in aggregated_usage.items():
        price = estimate_price(model)
        estimated_cost_usd += tokens * price
    estimated_cost_usd *= INFRASTRUCTURE_OVERHEAD_FACTOR
...
<body>
    ...
    <script>
        // FUNCIÓN switchTab ACTUALIZADA para los 4 identificadores
        ...
        function sendChat() { /* implementado en JS real */ }
...
async def chat_endpoint(request: ChatRequest) -> JSONResponse:
    ...
    try:
        system_prompt = (
            "Eres APA, un asistente para generar especificaciones de proyectos de software. "
            "Tu objetivo es ayudar al usuario a definir claramente qué quiere construir: "
            "objetivo, inputs, outputs esperados y criterios de éxito. "
            "Haz preguntas para clarificar si es necesario, pero mantén respuestas concisas."
        )
...
async def dashboard_endpoint(project_id: str) -> JSONResponse:
    """Endpoint para obtener métricas agregadas de un proyecto."""
    try:
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv:
        ...
        try:
            ...
            original_get_price = _get_model_price_per_token
            def mock_get_price(model: str) -> float:
                prices = {"qwen/qwen2.5-coder": 0.0, "anthropic/claude-3-5-sonnet": 0.000015}
                return prices.get(model, 0.0)

            import interface.app as app_module
            app_module._get_model_price_per_token = mock_get_price
            ...
            expected_cost = round(0.0 + (500 * 0.000015), 4)
            assert data["estimated_cost_usd"] == expected_cost
            assert "cache_entries" in data

            app_module._get_model_price_per_token = original_get_price
            print("✅ CRITERIO OK - Dashboard calcula costes dinámicamente")
